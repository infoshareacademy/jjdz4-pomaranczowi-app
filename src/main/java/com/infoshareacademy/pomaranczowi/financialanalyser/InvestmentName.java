...
class InvestmentName {
    ...
    ArrayList<String> investmentFundCode = new ArrayList<>();
    ...
    void loadInvestmentNameFromFile(String filepath) {
        ...
        try {
            while (fileScanner != null && fileScanner.hasNextLine()) {
                ...
                if (matcher.matches()) {
                    FilePath filePath = new FilePath(id,matcher.group(5) + matcher.group(6),matcher.group(7), matcher.group(5));
                    id++;
                    filePaths.add(filePath);
                    investmentNameMenu.add(matcher.group(7));
                    investmentFundCode.add(matcher.group(5));
                }
            }
